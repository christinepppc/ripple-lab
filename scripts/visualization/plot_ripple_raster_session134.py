#!/usr/bin/env python3
"""
Plot raster of detected and rejected ripples for Session 134 trial001
"""
...
# Load passed ripples
print(f"Loading Session {SESSION:03d} Trial {TRIAL:03d} data...")
...
print(f"Total ripples (passed rejection): {len(ripples_df)}")
print(f"Channels with ripples: {ripples_df['channel_id'].nunique()}")

# Get all channels sorted by region
all_channels = labels_df['bipolar_channel'].tolist()
channel_region_map = dict(zip(labels_df['bipolar_channel'], labels_df['primary_region']))
...
fig.suptitle(f'Session {SESSION:03d} Trial {TRIAL:03d} Ripple Raster Plot\\n(z_low=3.0, passed fast rejection)',
             fontsize=14, fontweight='bold')

tick_half = 0.4  # Half-height of raster tick

ax.set_xlabel('Time (seconds)', fontsize=12)
ax.set_ylabel('Bipolar Channel (sorted by region)', fontsize=12)
...
ax.legend(handles, labels_list, loc='upper right', ncol=2, frameon=True, fontsize=10)
...
output_file = output_dir / f"session{SESSION:03d}_trial{TRIAL:03d}_ripple_raster.png"
plt.savefig(output_file, dpi=150, bbox_inches='tight')
print(f"\nSaved raster plot to: {output_file}")

# Also save as PDF
output_pdf = output_dir / f"session{SESSION:03d}_trial{TRIAL:03d}_ripple_raster.pdf"
...
print(f"  Total passed ripples: {len(ripples_df)}")
print(f"  Channels with ripples: {ripples_df['channel_id'].nunique()}/{n_channels}")
